...
# --- Referentiels statistiques ---
REQUIREMENTS_STATISTIQUE = {
...
# Mapping domaine -> referentiels
REQUIREMENTS_BY_DOMAIN: dict[str, dict] = {
    "medical": REQUIREMENTS_MEDICAL,
    "statistique": REQUIREMENTS_STATISTIQUE,
}
...
def render_matrix_page():
    ...
    # Cles session state scopees par domaine
    data_key = f"matrix_data_{domain}"
    last_norme_key = f"_last_norme_{domain}"

    st.header(t("matrix_title"))
    st.caption(t("matrix_help"))

    # Choix du referentiel — ajouter option personnalisee
    standard_names = list(requirements.keys()) + [t("matrix_custom_empty")]
    norme = st.selectbox(
        t("matrix_standard"),
        standard_names,
        key=f"matrix_norme_{domain}",
    )

    # Reinitialiser la matrice si le referentiel change
    if st.session_state.get(last_norme_key) != norme:
        st.session_state[last_norme_key] = norme
        reqs = requirements.get(norme, [])
        st.session_state[data_key] = _get_empty_matrix(reqs)
    ...
    placeholder_key = f"matrix_search_placeholder_{domain}"
